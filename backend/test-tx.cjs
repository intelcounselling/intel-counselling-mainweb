...
async function test() {
  try {
    console.log('Testing transaction with undefined values...');
    const name = 'Test School ' + Date.now();
    const address = undefined; // undefined optional field
    const adminFirstName = 'Admin';
    const adminLastName = 'User';
    const adminEmail = 'admin_' + Date.now() + '@test.com';
    const adminPhone = undefined; // undefined optional field
